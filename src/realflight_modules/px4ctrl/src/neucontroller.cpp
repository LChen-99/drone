...
double NeuControl::fromQuaternion2yaw(Eigen::Quaterniond q)
{
  double yaw = atan2(2 * (q.x()*q.y() + q.w()*q.z()), q.w()*q.w() + q.x()*q.x() - q.y()*q.y() - q.z()*q.z());
  return yaw;
}

NeuControl::NeuControl(Parameter_t &param) : param_(param)
{
  resetThrustMapping();
}
...
NeuControl::calculateControl(const Desired_State_t &des,
...
*/
double 
NeuControl::computeDesiredCollectiveThrustSignal(
...
bool 
NeuControl::estimateThrustModel(
...
void 
NeuControl::resetThrustMapping(void)
{
